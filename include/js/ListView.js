...
{
	var view = 'ListViewContents';
		
	var hmode = jQuery("[id='hmode']").val();

	if(hmode == 1){
		var homeOwner = jQuery("[name='homeowner']").val();
		var hview = jQuery("[id='hview']").val();
		var exthmode = "&hmode=viewBox&hview="+hview+"&homeowner="+homeOwner;
		var view = hview;
	}
	...
	new Ajax.Request(
                ...
                {queue: {position: 'end', scope: 'command'},
                       	method: 'post',
                        postBody:"module="+module+"&action="+module+"Ajax&file=ListView&ajax=true&start=1&viewname="+viewName+"&parenttab="+parenttab+exthmode,
                        onComplete: function(response) {
                        $("status").style.display="none";
                        result = response.responseText.split('&#&#&#');
                        $(view).innerHTML= result[2];
