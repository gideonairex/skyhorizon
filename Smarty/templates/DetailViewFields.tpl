...
<!-- This file is used to display the fields based on the ui type in detailview -->
		{if $keyid eq '1' || $keyid eq 2 || $keyid eq '11' || $keyid eq '7' || $keyid eq '9' || $keyid eq '55' || $keyid eq '71' || $keyid eq '72' || $keyid eq '255'} <!--TextBox-->
			<td {$hidingStatField} width=25% class="dvtCellInfo" align="left">&nbsp;
						...
						<div id="currency_class" class="multiCurrencyDetailUI">
							...
							{foreach item=price key=count from=$PRICE_DETAILS}
								<tr>
									...
									<td {$hidingStatField} class="dvtCellLabel" width="40%">
										{$price.currencylabel} ({$price.currencysymbol})
									</td>
									<td {$hidingStatField} class="dvtCellInfo" width="60%" colspan="2">
										{$price.curvalue}
									</td>
								</tr>
							{/foreach}
		...
		{elseif $keyid eq '14'}<!--Time-->
			<td {$hidingStatField} width=25% class="dvtCellInfo" align="left">&nbsp;{$keyval}
			</td>
		{elseif $keyid eq '17'} <!--WebSite-->
			<td {$hidingStatField} width=25% class="dvtCellInfo" align="left">&nbsp;<a href="{$keyval}" target="_blank">{$keyval}</a>
			</td>
		{elseif $keyid eq '85'}<!--Skype-->
			<td {$hidingStatField} width=25% class="dvtCellInfo" align="left" id="mouseArea_{$label}">
		...
		{elseif $keyid eq '19' || $keyid eq '20'} <!--TextArea/Description-->
			...
			{/if}
			<td {$hidingStatField} width=100% class="dvtCellInfo" align="left">&nbsp;
		...
		{elseif $keyid eq '21' || $keyid eq '24' || $keyid eq '22'} <!--TextArea/Street-->
			<td {$hidingStatField} width=25% class="dvtCellInfo" align="left">&nbsp;<span id ="dtlview_{$label}">{$keyval}</span>
			</td>
		{elseif $keyid eq '50' || $keyid eq '73' || $keyid eq '51' || $keyid eq '57' || $keyid eq '59' || $keyid eq '75' || $keyid eq '81' || $keyid eq '76' || $keyid eq '78' || $keyid eq '80'} <!--AccountPopup-->
			<td {$hidingStatField} width=25% class="dvtCellInfo" align="left">&nbsp;<a href="{$keyseclink}">{$keyval}</a>
			</td>
		{elseif $keyid eq 82} <!--Email Body-->
			<td {$hidingStatField} colspan="3" width=100% class="dvtCellInfo" align="left">&nbsp;{$keyval}
			</td>
		...
		{elseif $keyid eq 83}<!-- Handle the Tax in Inventory -->
				...
				<td {$hidingStatField} colspan="2" class="dvtCellInfo">&nbsp;</td>
			</tr>
			<tr>
				<td {$hidingStatField} align="right" class="dvtCellLabel">
		...
		{elseif $keyid eq 69}<!-- for Image Reflection -->
			<td {$hidingStatField} align="left" width=25%">&nbsp;{$keyval}</td>
		{else}
			<td {$hidingStatField} class="dvtCellInfo" align="left" width=25%">&nbsp;{$keyval}</td>
		{/if}
