<?php /* Smarty version 2.6.18, created on 2014-09-17 20:29:38
         compiled from Settings/LayoutBlockList.tpl */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('modifier', 'vtiger_imageurl', 'Settings/LayoutBlockList.tpl', 448, false),)), $this); ?>
...
function getCreateFieldForm(modulename,blockid,mode)
{
   var check = validate(blockid);
   if(check == false)
   return false;
   ...
   var fldDecimal = document.getElementById("fldDecimal_"+blockid).value;
   var fldPickList = encodeURIComponent(document.getElementById("fldPickList_"+blockid).value);
   var fldRelation = document.getElementById("fldRelation_"+blockid).value;
   VtigerJS_DialogBox.block();
   new Ajax.Request(
		\'index.php\',
		{queue: {position: \'end\', scope: \'command\'},
			method: \'post\',
			postBody: \'module=Settings&action=SettingsAjax&file=LayoutBlockList&sub_mode=addNotCustomField&fld_module=\'+modulename+\'&ajax=true&blockid=\'+blockid+\'&fieldType=\'+type+\'&fldLabel=\'+label+\'&fldLength=\'+fldLength+\'&fldDecimal=\'+fldDecimal+\'&fldPickList=\'+fldPickList+\'&fldRelation=\'+fldRelation,
			onComplete: function(response) {
				VtigerJS_DialogBox.unblock();
				var str = response.responseText;
				if(str == \'ERROR\'){
...
function makeFieldSelected(oField,fieldid,blockid)
{
	...
	if(fieldid == \'relation\'){
		document.getElementById("fieldType_"+blockid).value = \'relation\';
		document.getElementById(\'selectedfieldtype_\'+blockid).value = \'relation\';	
		
		document.getElementById("lengthdetails_"+blockid).style.visibility="hidden";
		document.getElementById("decimaldetails_"+blockid).style.visibility="hidden";
		document.getElementById("picklistdetails_"+blockid).style.visibility="hidden";
		document.getElementById("relateddetails_"+blockid).style.visibility="visible";
	}
	else{
