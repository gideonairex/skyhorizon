...
$block = getBlocks($currentModule,"detail_view",'',$focus->column_fields);
//ed edited
$supremeAdmins = getSupremeAdmins(); 	//include/utils/CommonUtils.php
if(!in_array($block['User Login & Role'][0]['User Name']['value'], $supremeAdmins) || $current_user->isSupreme){
	...
	if (is_admin($current_user))
	{
		...
		if($_REQUEST['record'] != $current_user->id)
		{
			...
			$buttons = "<input type=\"button\" onclick=\"deleteUser($focus->id);\" class=\"crmButton small cancel\" value=".$app_strings['LBL_DELETE_BUTTON_LABEL']."></input>";
