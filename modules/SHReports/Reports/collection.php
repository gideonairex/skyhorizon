<?php
	...
	$query = "select *, vtiger_collection.collection_no as `collection_no_`,
		...
		left join vtiger_salesagreement on vtiger_salesagreement.salesagreementid = vtiger_accountsreceivable.sales_no
	...
	if($num_rows == 0){
		...
		for( $i = 0 ; $i < $num_rows; $i++){
			...
			if( $adb->query_result( $result, $i, "nontraderid" ) == '' ) {
				...
				$data['summary'][$method]['details'][$collection_no]['lists'][$i]['sa_no'] = "<a href='index.php?module=NonTradeR&action=DetailView&record=".$adb->query_result($result, $i, "nontraderid")."' target=_blank >".$adb->query_result($result, $i, "ntr_no")."</a>";
				$data['summary'][$method]['details'][$collection_no]['lists'][$i]['sa_no_'] = $adb->query_result($result, $i, "ntr_no");
			}
