...
elseif($subMode == 'addNotCustomField')
	$duplicate = addNotCustomField();
...
//ed edited
$smarty->assign("POSSIBLE_RELATION",getlistofpossiblerelation());
//ed edited end
 ...
 */
 ...
 //ed edited
 function getlistofpossiblerelation(){
	global $adb;
	$sql ="select modulename from vtiger_entityname order by modulename asc";
...
function addNotCustomField(){
	...
	if($adb->num_rows($checkresult) > 0 ) {
		...
		//echo "<br/>ed-";
		//echo strtolower($fldlabel);
		//echo $columnName= str_replace(' ','_',strtolower($fldlabel));
		$columnName= preg_replace("![^a-z0-9]+!i", "_", strtolower($fldlabel));
		//echo "<br/>max_fieldid-";
		
		if($fldType == 'Picklist' || $fldType == 'MultiSelectCombo') {
			$checkexisttable = "show tables like 'vtiger_$columnName'";
			$existtable=$adb->pquery($checkexisttable,array());
			if($existtable->fields != '' && $existtable->fields != ' '){
				//echo "IN";
				return "yes";
			}
		}
		...
		if(is_numeric($blockid)) {
			if($_REQUEST['fieldid'] == '') {
				...
				}else if($fldType == 'relation'){
					$sql ="select fieldid from vtiger_field where fieldlabel = '$fldlabel' and tabid = '$tabid'";
					$temp_fieldid = $adb->pquery($sql, array());
					$sql = "insert into vtiger_fieldmodulerel (fieldid,module,relmodule) values(?,?,?)";
					$adb->pquery($sql, array($temp_fieldid->fields['fieldid'],$_REQUEST['fld_module'],$_REQUEST['fldRelation']));					
				}
			}
		}
	}
