...
$smarty->assign("HOMEOWNERID",$_REQUEST['homeowner']);
...
{
	if(method_exists($focus,'addFilter'))
		$query .= $focus->addFilter($_REQUEST['srcmodule']);
	
    $query .= ' ORDER BY '.$order_by.' '.$sorder;
}else{
	if(method_exists($focus,'addFilterWithOrder'))
		$query .= $focus->addFilterWithOrder($_REQUEST['srcmodule']);
}
